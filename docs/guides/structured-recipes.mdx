...
A highlight that follows the shape of the glyphs. The window element sweeps one
way and the copy inside counter-translates by the same amount, so the glyphs
stay still on screen and the whole loop is one transform.
...
<Warning>
The element must carry no padding of its own — put it on a parent. The sweep is
positioned against the padding box, and the window and the copy only stay in
step while they are the same width.
</Warning>

The copy is `aria-hidden`, so the text is announced once. Writing
...
[Render cost](/docs/reference/render-cost#tm-shimmer-text).
