...
description: "Why 39 keyframes moved off `transform`, which ones did not, and what still cannot stack."
---
...
TailMotion ships 90 keyframes. 15 of them touch only opacity, filter, colour or
clip-path and never had a transform to conflict with. Of the 75 that move an
element, **39 now animate the individual properties**:
...
## Keep hover hit targets stationary

A hover-triggered keyframe that moves or shrinks its own element can move the
element's hitbox out from under the pointer. That removes `:hover`, resets the
animation, returns the element under the pointer, and starts the cycle again.

Put the hover state on a stationary parent and the motion on its child:

```html
<div class="group">
  <div class="group-hover:tm-slide-block-start">
    Stable hover target, moving content
  </div>
</div>
```

TailMotion ships a matching `group-hover:` form for every prebuilt `hover:`
variant. Direct `hover:` remains useful for opacity-only effects and for cases
where the element's hitbox does not move.
...
- A motion class can be added to a Tailwind-transformed element without silently
  removing the intended transform — for the 39 keyframes listed above and every
  transition-based class.
- The 36 that still overwrite `transform` are named, grouped by reason, and have
  a documented wrapper workaround.
- `npm run check` recounts all three numbers on every build and fails if this
  page drifts from the stylesheet.
