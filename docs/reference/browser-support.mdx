...
| Individual `translate` / `scale` / `rotate` | 39 keyframes, every presence class | 104 | 14.1 | 72 | — |
...
| `mask-image` | the `tm-shimmer-text` sweep window | 120 | 15.4 | 53 | Prefixed `-webkit-mask-image` is shipped alongside and covers older Chrome and Safari |
| `:has()` | telling the `tm-shimmer-text` sweep recipe from the zero-markup path | 105 | 15.4 | 121 | The zero-markup path does not animate. Text renders ordinary and fully visible; the sweep recipe is unaffected |
...
- No blur in the library exceeds 8px, except `tm-text-rotate` at 10px and
  `tm-text-morph` at 12px, where the blur is the word-to-word transition
  itself rather than decoration. Both are one-shot, on one inline element, and
  never loop. Large blur radii are expensive everywhere and worst in Safari.
  `npm run check` fails the build on any other blur above 8px — see
  [Render cost](/docs/reference/render-cost#blur).
...
rather than a promise. The same command checks where each class does its work —
see [Render cost](/docs/reference/render-cost).
