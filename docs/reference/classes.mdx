...
| `tm-stagger-blur` | Adds an 8px blur to the entrance and exit. Off by default | `tm-stagger` |
...
| `tm-glow` | 1600ms | halo via `--tm-glow-color`, on the element's `::after` |
| `tm-morph` | 4000ms | subtle scale + rotate |
| `tm-ripple` | 1800ms | expanding ring, on the element's `::after` |
| `tm-shimmer` | 1600ms | gradient sweep on a pseudo-element |
| `tm-shimmer-text` | 2000ms | highlight sweeping across the text. Needs the sweep child — see [Render cost](/docs/reference/render-cost#tm-shimmer-text) |
| `tm-sparkle` | 1400ms | sparkle |

<Note>
`tm-glow` and `tm-ripple` own the element's `::after` and set
`position: relative` on it, so the loop can run on the compositor rather than
repainting a `box-shadow` every frame. `tm-shimmer-text` takes a sweep child
for the same reason. All three keep their existing tokens. See
...
Every bundled `hover:tm-*` class also has a `group-hover:tm-*` equivalent. Prefer
the group form when an animation translates, scales or rotates its subject:

```html
<div class="group">
  <div class="group-hover:tm-pop">Content</div>
</div>
```

The parent remains a stationary pointer target while the child moves. Applying
`hover:tm-pop` directly to the moving child can make the cursor leave its
transformed hitbox, remove `:hover`, reset the animation, and immediately start
it again when the child returns.
